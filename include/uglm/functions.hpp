...
namespace u
{
	...
	inline GLfloat length(area A) { return GLfloat(glmSqrt(A)); }
	// Calculating the length of a 2D vector.
	inline GLfloat length(vec2 V) { return GLfloat(glmSqrt(pow2(V.x) + pow2(V.y))); }
	// Calculating the length of a 3D vector.
	inline GLfloat length(vec3 V) { return GLfloat(glmSqrt(pow2(V.x) + pow2(V.y) + pow2(V.z))); }
	// Calculating the length of a 4D vector.
	inline GLfloat length(vec4 V) { return GLfloat(glmSqrt(pow2(V.x) + pow2(V.y) + pow2(V.z) + pow2(V.w))); }
